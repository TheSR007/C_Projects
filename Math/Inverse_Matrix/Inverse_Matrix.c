...
void printMatrix(double matrix[10][10], double inverse[10][10], int n, int showIdentity);
void gaussJordan(double matrix[10][10], double inverse[10][10], int n, int showSteps);
...
int main() {
    ...
    Start:
    printf("Enter the order of the matrix (n x n)[max 10]: ");
    scanf("%d", &n);
    if(n>10){
        printf("n must be <11\n");
        goto Start;
    ...
    // Step 3: Ask the user if they want to see each step
    printf("Show Steps? (1 for Yes, 0 for No): ");
    scanf("%d", &showSteps);

    // Step 4: Initialize the identity matrix for the inverse
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
    ...
    // Step 5: Check if the matrix is singular (determinant = 0)
    ...
    // Step 6: Print the initial matrices (Matrix A and Identity Matrix)
    printf("\nInitial matrices:\n");
    ...
    // Step 7: Perform Gauss-Jordan elimination with optional step display
    gaussJordan(matrix, inverse, n, showSteps);

    // Step 8: Print the final inverse matrix
    printf("\nThe inverse matrix is:\n");
    printMatrix(inverse, 0, n, 0);  // Only print the final inverse matrix, not the identity side
...
int findNonZeroPivot(double matrix[10][10], int n, int col) {
...
void showsteps(double matrix[10][10], double inverse[10][10], int n) {
